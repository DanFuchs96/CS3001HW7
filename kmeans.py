...
def loadCSV(fileName, remove_header=True):
...
DISTANCE_MODES = ['SoS', 'Euclidean', 'Cosine', 'Jaccard', 'Manhattan']
...
def distance(instance1, instance2):
    if instance1 == None or instance2 == None:
        return float("inf")
    distance_sum = 0
    if DISTANCE_MODE == DISTANCE_MODES[0]:  # Sum of Squares
        for i in range(1, len(instance1)):
            distance_sum += (instance1[i] - instance2[i]) ** 2
    elif DISTANCE_MODE == DISTANCE_MODES[1]:  # Euclidean
        for i in range(1, len(instance1)):
            distance_sum += (instance1[i] - instance2[i]) ** 2
        distance_sum = distance_sum ** 0.5
    elif DISTANCE_MODE == DISTANCE_MODES[2]:  # 1 - Cosine
        i1_sum = 0
        i2_sum = 0
        for i in range(1, len(instance1)):
            distance_sum += (instance1[i] * instance2[i])
            i1_sum += (instance1[i] ** 2)
            i2_sum += (instance2[i] ** 2)
        i1_sum = i1_sum ** 0.5
        i2_sum = i2_sum ** 0.5
        distance_sum = 1 - (distance_sum / (i1_sum * i2_sum))
    elif DISTANCE_MODE == DISTANCE_MODES[3]:  # 1 - Jaccard
        min_sum = 0
        max_sum = 0
        for i in range(1, len(instance1)):
            min_sum += min(instance1[i], instance2[i])
            max_sum += max(instance1[i], instance2[i])
        distance_sum = 1 - (min_sum/max_sum)
    elif DISTANCE_MODE == DISTANCE_MODES[4]:  # Manhattan
        for i in range(1, len(instance1)):
            distance_sum += abs(instance1[i] - instance2[i])
    else:
        print("Invalid Distance Mode Selected")
    return distance_sum
...
def kmeans(instances, k, animation=False, initCentroids=None, distance_type='SoS'):
    global DISTANCE_MODE
    DISTANCE_MODE = distance_type
    result = {}
